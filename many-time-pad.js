...
for (let i = 0; i < input[10].length; i += 2) {
    let isDecoded = false;
    for (let j = 0; j < 10; j++) {
        let s1 = '0x' + input[j][i] + input[j][i + 1];
        let s2 = '0x' + input[10][i] + input[10][i + 1];
        let strXOR = String.fromCharCode(s1 ^ s2 ^ 0x20)

        // Kiểm tra xem ký tự vủa XOR có phải là chữ cái không
    ...
    // Nếu chưa giải mã được, thêm dấu * vào kết quả
    if (!isDecoded) {
        targetPlain += "*";
    } 
}

console.log(targetPlain)

for (let i = 0; i < input[10].length / 2; i++) {
    if (targetPlain[i] == '*') {
        let tmp = "";
        let s2 = '0x' + input[10][i * 2] + input[10][i * 2 + 1];
        for (let j = 0; j < 10; j++) {
            let s1 = '0x' + input[j][i * 2] + input[j][i * 2 + 1];
            tmp += String.fromCharCode(s1 ^ s2 ^ 0x20) + " "
        }
        console.log("position " + i + " can be: " + tmp)
    }
}